...
public class FileServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		try {
			...
			FileType fileType = FileType.valueOf(pathInfoArr[0]); // Expect it to be a FileType.TYPE_SOFTWARE/TYPE_SCRIPT
			String firmwareVersion = pathInfoArr[1];
			unittypeName = pathInfoArr[2];
			if (pathInfoArr.length > 3) // The optional unit-id is also sent - only for logging purpose
			...
			if (firmwareImage == null || firmwareImage.length == 0) {
				...
				res.setContentLength((int) firmwareImage.length);
